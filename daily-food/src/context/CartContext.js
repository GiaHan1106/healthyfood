...
const CartProvider = ({ children }) => {
    ...
    const addCart = (item, slug, title) => {
        const checkCart = cart.find((product) => product.id === item.daymenu_id && product.slug === slug);
        if (!checkCart) {
    ...
    const addCartRetail = (food) => {
        const checkCartRetail = cartRetail.findIndex((item) => item.foodmenu_id === food.foodmenu_id);
    ...
    const deleteCartRetail = (id) => {
        const indexProduct = cartRetail.findIndex((item) => id === item.foodmenu_id);
        if (indexProduct === -1) {
            toast.error("Item not found in retail cart!", {
                position: "top-center",
            });
            return;
        }
    ...
    const deleteAll = () => {
        ...
        localStorage.removeItem("CARTRETAIL");
    };
