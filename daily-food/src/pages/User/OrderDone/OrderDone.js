...
const OrderDone = () => {
    ...
    useEffect(() => {
        const filteredOrders = listOrder.filter((order) => {
            ...
            return userInfo.email === user.email && order.status === "Done";
        });
                    ...
                    <tbody>
                        {sortedOrders.length > 0 ? (
                            sortedOrders.map((item) => {
                                ...
                                const isDone = item.status === "Done"; // Kiểm tra nếu trạng thái là 'Done'
                                return (
                                    <tr key={item.id}>
                                        ...
                                        <td>
                                            <button
                                                style={{
                                                    backgroundColor: isDone ? "gray" : canCancel ? "red" : "gray", // Đổi màu khi trạng thái là Done
                                                    color: "white",
                                                    borderRadius: "5px",
                                                    cursor: isDone ? "not-allowed" : canCancel ? "pointer" : "not-allowed", // Vô hiệu hóa khi 'Done'
                                                }}
                                                onClick={() => !isDone && canCancel && handleCancel(item.id)} // Không cho phép hủy khi trạng thái là 'Done'
                                                disabled={isDone || !canCancel} // Nếu trạng thái là 'Done', nút sẽ bị vô hiệu hóa
                                            >
                                                Cancel
                                            </button>
                                        </td>
